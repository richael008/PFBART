...
void mbart(int *iNumObs, int *iNumX, int *inrowTest,
	   ...
	   int *verbose,
	   double *sdraw, double *trdraw, double *tedraw, int *vcdraw,int *FixFirst,int *ChangePrior,int *AllowSwap,int *StopDeath)
{
   GetRNGstate();
   
   
   IFix=*FixFirst;
   IChangePrior=*ChangePrior;

   IAllowSwap=*AllowSwap;
   IStopDeath=*StopDeath;
   Rprintf("ChangePrior: %d\n",IChangePrior);
   Rprintf("IAllowSwap: %d\n",IAllowSwap);
   Rprintf("IStopDeath: %d\n",IStopDeath);    
   ...
   CrossM = new int*[NumX+1];

   for(int i=0;i<=NumX;i++)
     CrossM[i]=new int[NumX+1];  
