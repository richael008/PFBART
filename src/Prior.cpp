...
int SumFixGoodVar(Node *n)
{
	int sum = 0;
	int i;
	for(i=1;i<=IFix;i++) sum += (n->VarAvail)[i];
	return sum;
}
...
double PGrow(Node *n)
{
	...
	if(Ngood) {
		...
		if (IChangePrior) {
			if(nobs<5) {
				return .001*alpha/(pow(1.0+(Depth(n)<=IFix ? 0:(Depth(n)-IFix)) ,beta));
				} 
			else {
			//return .9;
			return alpha/(pow(1.0+(Depth(n)<=IFix ? 0:(Depth(n)-IFix)) ,beta));
				}
		}
		else {
			if(nobs<5) {
...
// returns index of variable to split on
{
	int Ngood = SumFixGoodVar(n);
	//double u=ran1(&idum);
	double u= unif_rand();
	int VarI =  (int)floor(u*Ngood)+1;

	return GetSkipBadInd(IFix,n->VarAvail,VarI);
